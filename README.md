# Histology viewer

## Описание

Histology viewer - учебный веб-сайт для работы с цифровыми гистологическими
препаратами. Он помогает студентам изучать микропрепараты в браузере,
масштабировать изображение, просматривать описание препарата и проходить
диагностические задания.

Сайт включает пользовательскую часть для просмотра материалов и прохождения
тестирования, а также административный интерфейс для управления препаратами,
диагностиками и результатами. Backend хранит данные в PostgreSQL, а frontend
построен на React и Vite.

## Версии проекта

### main — обычная версия

- локальная авторизация;
- обычная админка;
- Moodle не требуется;
- преподаватели и роли управляются в админке.

### test — Moodle/LTI версия

- вход через Moodle/LTI;
- роли и permissions такие же, как в обычной версии;
- Moodle используется как источник авторизации;
- локальный администратор сохраняется для обслуживания сайта.

## Роли и права

Роли внутри сайта единые для локального и Moodle/LTI режимов:

- `admin` — администратор, полный доступ, управление пользователями и ролями;
- `teacher_full` — преподаватель с полным доступом к препаратам, диагностикам и результатам;
- `teacher_limited` — преподаватель с ограниченным доступом: диагностики и результаты без управления препаратами;
- `resident` — ординатор, обезличенный просмотрщик препаратов без названий, диагнозов и описаний;
- `student` — студент, просмотр доступных материалов и прохождение диагностик без доступа к админке.

Итоговые права считаются как базовая роль плюс индивидуальные overrides пользователя:

```js
finalPermissions = {
  ...ROLE_PERMISSIONS[user.role],
  ...user.permissionOverrides
}
```

Администратор управляет локальными пользователями в разделе `Пользователи`: создаёт учётную запись, выбирает роль, включает или отключает конкретные permissions, меняет пароль и блокирует пользователя.

## Настройки .env

Для обычной версии `main`:

```env
AUTH_MODE=local
ENABLE_MOODLE_LTI=false
```

Для Moodle/LTI версии `test`:

```env
AUTH_MODE=moodle_lti
ENABLE_MOODLE_LTI=true
```

При `AUTH_MODE=local` сайт открывается напрямую, `/api/me` может вернуть гостя, а DZI-тайлы открываются без Moodle-сессии. При `AUTH_MODE=moodle_lti` Moodle используется как способ входа, но права берутся из общей системы ролей.


## PostgreSQL

Backend хранит препараты, диагностики и результаты тестирования в PostgreSQL.
По умолчанию используется подключение:

```bash
postgres://postgres:postgres@127.0.0.1:5432/histology_viewer
```

Перед запуском backend поднимите PostgreSQL локально или используйте внешнюю БД.
Если параметры подключения отличаются от значения по умолчанию, задайте:

```bash
export DATABASE_URL="postgres://user:password@host:5432/database"
```

При первом старте backend создаёт таблицы и переносит данные из старых JSON-файлов
`server/data/slides.json`, `server/data/diagnostics.json`,
`server/data/diagnostic-results.json`, если соответствующие таблицы пустые.

## Авторизация админки

Административный интерфейс и все `/api/admin/*` маршруты требуют пароль. На
сервере задайте переменную окружения:

```bash
export ADMIN_PASSWORD="сложный-пароль"
```

При необходимости отдельного доступа преподавателя задайте второй пароль:

```bash
export TEACHER_PASSWORD="пароль-преподавателя"
```

Для отдельных логинов преподавателей используйте `TEACHER_ACCOUNTS` вместо
общего пароля. Пример для `systemd` через `EnvironmentFile`:

```ini
# /etc/histology-viewer.env
ADMIN_LOGIN=admin
ADMIN_PASSWORD=сложный-пароль-администратора
TEACHER_ACCOUNTS='[{"login":"ivanov","password":"пароль-иванова"},{"login":"petrova","password":"пароль-петровой"}]'
ADMIN_SESSION_SECRET=длинный-постоянный-секрет
```

В override сервиса добавьте `EnvironmentFile=/etc/histology-viewer.env`, затем
выполните `sudo systemctl daemon-reload` и `sudo systemctl restart histology-viewer`.
Логины могут содержать латинские буквы, цифры, `_` и `-`. Старый
`TEACHER_PASSWORD` сохраняет совместимый вход с логином `teacher`.

После первого входа администратором новые учётные записи лучше создавать во
разделе «Пользователи» админ-панели. Они сохраняются в PostgreSQL, поэтому
для добавления, смены пароля, настройки прав и блокировки пользователя не требуется менять
`systemd` или перезапускать сервис.

Преподаватель может работать с препаратами, диагностиками и результатами, но
не может восстановить backup. `ADMIN_PASSWORD` сохраняет полный доступ.

Дополнительно можно задать постоянный секрет подписи cookie-сессий:

```bash
export ADMIN_SESSION_SECRET="длинная-случайная-строка"
```

На сервере задавайте оба значения постоянно. Если `ADMIN_SESSION_SECRET` не
задан, backend сгенерирует временный секрет при старте, и все админские сессии
будут сбрасываться после перезапуска.

Для локальной проверки можно запустить backend с временным паролем:

```bash
ADMIN_PASSWORD="test123" ADMIN_SESSION_SECRET="local-dev-secret" npm run dev
```

Если `ADMIN_PASSWORD` не задан, вход в админку будет отключён.

## Форматы препаратов

Основной формат для просмотра в браузере - DZI. Админка умеет:

- принимать готовый DZI ZIP с `.dzi` и папкой `*_files`;
- конвертировать `SVS`, `TIFF`, `NDPI`, `SCN`, `MRXS` через `vips dzsave`;
- принимать `.kfb`, но реальная конвертация зависит от серверного декодера.

Для тяжёлых и нестандартных препаратов предпочтительно заранее экспортировать
готовый DZI ZIP и загрузить его в админке. Для `MRXS` загружайте ZIP с файлом
`.mrxs` и связанной папкой данных. Для `KFB`, если `vips/OpenSlide` не
распознаёт файл, экспортируйте препарат из программы сканера в `DZI`, `TIFF`
или `SVS`, либо установите отдельный KFB-конвертер на сервер.

### Массовое добавление карточек

Во вкладке «Препараты» админ-панели можно загрузить CSV или JSON с карточками,
которые уже ссылаются на готовые DZI. Обязательные поля каждой карточки:
`title`, `lesson`, `system`, `organ`, `stain`, `source`. `id` необязателен и
будет сформирован из названия. Занятые ID не перезаписываются: импорт покажет
строки, которые были пропущены. Диагностические признаки и вопросы для
самопроверки можно задать полями `diagnosticSigns` и `selfCheckQuestions`.

Пример CSV (разделитель `;`):

```csv
id;title;lesson;system;organ;stain;source;description
infarkt-myokarda;Инфаркт миокарда;Занятие 1;Сердечно-сосудистая система;Сердце;H&E;/slides/infarkt-myokarda.dzi;Коагуляционный некроз миокарда
```

Ошибки конвертации сохраняются в `server/data/upload-logs`, а резервные копии
данных можно создавать и восстанавливать во вкладке `Backups` в админке.
Backend также создаёт полный backup карточек, диагностик и результатов каждые
24 часа. Интервал и срок хранения задаются переменными `BACKUP_INTERVAL_HOURS`
и `BACKUP_RETENTION_DAYS` (по умолчанию 24 и 30). Каталоги `raw-slides` и
`public/slides` содержат большие файлы препаратов и должны резервироваться на
сервере отдельно, например ежедневным `rsync` в хранилище backup.

Тяжелая конвертация `vips dzsave` выполняется отдельным Node worker-процессом.
Задачи обрабатываются последовательно: это не блокирует HTTP-сервер и не дает
нескольким большим препаратам одновременно занять память и диск.

## Запуск

В папке `server`:

```bash
npm run dev
```

Во втором терминале запустите frontend:

```bash
npm run dev -- --host 0.0.0.0
```

## Проверка состояния

Backend отдаёт диагностический endpoint:

```bash
curl http://127.0.0.1:4000/api/health
```

Если всё работает, ответ вернётся с HTTP 200 и `ok: true`. В ответе есть
состояние PostgreSQL, счётчики препаратов, диагностик, результатов, права на
папки `raw-slides`, `public/slides`, `server/data/upload-logs`, состояние
`vips/OpenSlide`, очередь worker (`conversion.worker`) и количество готовых
DZI-файлов в `public/slides`.
`adminAuth.configured` показывает, задан ли пароль администратора, а
`adminAuth.sessionSecretConfigured` - задан ли постоянный секрет cookie-сессий.

## Moodle LTI 1.3 / LTI Advantage

Пользовательская часть атласа запускается из Moodle. Задайте `LTI_*` переменные
из [.env.example](.env.example): ключи инструмента, issuer, client/deployment ID,
OIDC/token/JWKS Moodle и публичный `LTI_REDIRECT_URI`. Приватный ключ хранится
только в окружении сервера.

В Moodle создайте External Tool:

```text
Tool name: Гистологический атлас
LTI version: LTI 1.3
Tool URL: https://example.com/lti/launch
Login initiation URL: https://example.com/lti/login
Redirect URI: https://example.com/lti/launch
JWKS URL: https://example.com/.well-known/jwks.json
```

Включите сервис Assignment and Grade Services и выдайте scope lineitem/score.
После сдачи диагностики результат сохраняется локально в `lti_grade_results` и
отправляется в Gradebook; при ошибке сохраняются `status=failed`, `attempts` и
`last_error`. Администратор повторяет отправку: `POST /api/admin/results/:id/resend-to-moodle`.

Если Moodle не передаёт группу в LTI claims, добавьте custom parameters:
`custom_moodle_course_id`, `custom_moodle_course_shortname`,
`custom_moodle_group_id`, `custom_moodle_group_name`, `custom_user_group`.
Пользователь без группы не получает материал, ограниченный группой.

Роли: `admin`, `teacher_full`, `teacher_limited`, `resident`, `student`.
Ограниченный преподаватель не может изменять/загружать/удалять слайды;
ординатор получает обезличенный список и viewer без описаний и меток. Backend
проверяет права независимо от скрытия элементов интерфейса.

### Защита DZI через Nginx

В production не публикуйте `public/slides` открытым location. Пример:

```nginx
location /slides/ {
  auth_request /_slide_access;
  alias /srv/histology-viewer/public/slides/;
}
location = /_slide_access {
  internal;
  proxy_pass http://127.0.0.1:4000/api/auth/check-slide-access;
  proxy_pass_request_body off;
  proxy_set_header Content-Length "";
  proxy_set_header Cookie $http_cookie;
  proxy_set_header X-Original-URI $request_uri;
}
```

В development backend сам защищает `/slides`. Для проверки откройте инструмент
из Moodle и убедитесь, что `/api/me` возвращает `authProvider: moodle_lti`.
Для resident проверьте отсутствие названия/описания в `/api/slides`; для
`teacher_limited` — 403 на POST/DELETE `/api/admin/slides` и доступ к POST
`/api/admin/diagnostics`.

### Локальный просмотр без Moodle

Только для локальной разработки можно включить `DEV_AUTH_BYPASS=true` и
выбрать роль через `DEV_AUTH_ROLE` (например, `teacher_full`, `student` или
`resident`). Также доступны `DEV_AUTH_COURSE_ID` и `DEV_AUTH_GROUP_ID`.
Backend создаёт mock-пользователя для `/api/me` и всех middleware, поэтому
можно проверить каталог, viewer, фильтры и группы без Moodle. **Никогда не
включайте этот параметр на сервере:** при `NODE_ENV=production` bypass
принудительно игнорируется.

```bash
DEV_AUTH_BYPASS=true DEV_AUTH_ROLE=teacher_full npm --prefix server run dev
```

## Разработка и деплой

Код хранится в GitHub, а ветка `main` считается стабильной версией для сервера.
Изменения лучше делать в отдельных ветках и вливать в `main` через Pull Request.

Перед отправкой изменений:

```bash
npm run lint
npm run build
```

На сервере после обновления `main`:

```bash
./scripts/deploy.sh
```

Подробнее: `docs/development-workflow.md`.
